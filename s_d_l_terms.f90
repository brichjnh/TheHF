...
integer(i4b) :: ab, cd, za, zb, zc, zd, zzab, zzcd, m, jrec
real(dp) :: ooppq, qpq, poq, delv(3),qoppq, rpq2
real(dp) :: rba(3), rdc(3), wx(3)
real(dp) :: bfac(0:5), osv(56,0:5),oset(56,10), rap(3), rpw(3)
...
do zzcd=1,nksqu(cd)
    ...
    dlllc(:,:,2:4,2:4)=dlllc(:,:,2:4,2:4)+oshk(:,:,2:4,2:4)*ddabl(zzcd,cd)
end do
...
do zzab=1,nksqu(ab)
    ...
    do zzcd=1,nksqu(cd)
        ...
        ddllc(:,:,2:4,2:4)=ddllc(:,:,2:4,2:4)+oshk(:,:,2:4,2:4)*ddabl(zzcd,cd)
    end do
end do
...
integer(i4b) :: j,l,jm1,lm1,li,lf, bigL, i, ip1, jj, jp1, k, kp1 , lp1
integer(i4b) :: ab, cd, zb, zd, zzab, zzcd, m, jrec
real(dp) :: ooppq, qpq, poq, delv(3), qoppq, rpq2
real(dp) :: rba(3), rdc(3), wx(3)
real(dp) :: bfac(0:6), osv(84,0:6),oset(84,20), rap(3), rpw(3)
real(dp) :: oshb(20,4,20), oshk(6,4,20,4), dldlc(6,4,6,4)
...
integer(i4b) :: j, l, jm1, lm1, li, lf, bigL, i, jj, jp1, k, kp1 , lp1
integer(i4b) :: ab, cd, zb, zd, zzab, zzcd, m, jrec
real(dp) :: ooppq, qpq, poq, delv(3), qoppq, rpq2
real(dp) :: rba(3), rdc(3), wx(3)
...
real(dp) :: bfac(0:3), os(10,4,0:3), eterm(6,4),osc(6,4), rdq(3), rqw(3), rcq(3)
...
do zzcd=1,nksqu(cd)
    ...
    do zzab=1,nksqu(ab)
        ...
        afac(0:1)=hpm1(zzcd,cd)*(os(1,1,0:1)-poppq*os(1,1,1:2))
        do jj=1,3
          kp1=jj+1
        ...
        do kp1=5,10
          ...
          km1=minus1(jrec,k)
          if (km1.ne.0) then
            os(kp1,:,0)=os(kp1,:,0)+hpm1(zzcd,cd)*(os(km1,:,0)-poppq*os(km1,:,1))
          end if
          os(kp1,jrec+1,0)=os(kp1,jrec+1,0)+hpm1(zzcd,cd)*(os(k,1,0)-poppq*os(k,1,1))
        end do
        eterm=eterm+os(5:10,:,0)
    end do
    osc(:,1)=osc(:,1)+eterm(:,1)
    osc(:,2:4)=osc(:,2:4)+eterm(:,2:4)*ddabl(zzcd,cd)
end do
...
integer(i4b) :: j, l, jm1, jj, jp1, kk, jrec
integer(i4b) :: ab, cd, zzab, zzcd, m
real(dp) :: ooppq, qpq, qoppq, rpq2, wx(3)
...
do zzcd=1,nksqu(cd)
      ...
      osc(:,1)=osc(:,1)+ketc(:,1)
      osc(:,2:4)=osc(:,2:4)+ketc(:,2:4)*ddabl(zzcd,cd)
end do
...
integer(i4b) :: ab, cd, za, zd, zzab, zzcd, m
    ...
    do zzcd=1,nksqu(cd)
        ...
        do l=1,20
          do jj=1,3
            ...
            oset(jp1,l)=oset(jp1,l)*ddabl(zzab,ab)
          end do
        end do
        ...
        do k=5,10
          do jj=1,3
            lp1=jj+1
            kp1=plus1(jj,k)
            osh(:,k,lp1)=ddabl(zzcd,cd)*(osh(:,kp1,1)+rdc(jj)*osh(:,k,1))
          end do
        end do
        osc=osc+osh(:,5:10,:)
    end do
...
integer(i4b) :: ab, cd, za, zc, zd, zzab, zzcd, m, jrec
real(dp) :: ooppq, qpq, qop, poppq, rpq2
...
do zzab=1,nksqu(ab)
    ...
    do zzcd=1,nksqu(cd)
        ...
        osc(:,2:4,2:4)=osc(:,2:4,2:4)+osh(:,2:4,2:4)*ddabl(zzcd,cd)
    end do
end do
...
integer(i4b) :: j,l, lm1,lp1, k, kp1, bigL, jj, jp1, li, lf, jrec
integer(i4b) :: ab, cd, za, zb, zc, zzab, zzcd, m
real(dp) :: ooppq, qpq, qop, poppq, rpq2
...
integer(i4b) :: ab, cd, za, zc, zd, zzab, zzcd, m
real(dp) :: ooppq, qpq, qop, poppq, rpq2
...
do zzcd=1,nksqu(cd)
    ...
    esssl(2:4)=esssl(2:4)+osc(2:4)*ddabl(zzcd,cd)
end do
return
...
integer(i4b) :: ab, cd, zc, zd, zzab, zzcd, m
integer(i4b) :: k, jj
...
do zzcd=1,nksqu(cd)
    ...
    essll(2:4,2:4)=essll(2:4,2:4)+osc(2:4,2:4)*ddabl(zzcd,cd)
end do
...
real(dp) :: bfac(0:2), osc(4,4), os(4,4,0:2), rbp(3), rpw(3), rdq(3), rqw(3)
...
do zzab=1,nksqu(ab)
    ...
    do zzcd=1,nksqu(cd)
        ...
        fac1=.5_dp*os(1,1,1)*ooppq
        do j=1,3
          os(1+j,2:4,0)=(rbp(j)*os(1,2:4,0)+rpw(j)*os(1,2:4,1))
          os(j+1,j+1,0)=os(j+1,j+1,0)+fac1
        end do
        osc(:,1)=osc(:,1)+os(:,1,0)
        osc(:,2:4)=osc(:,2:4)+os(:,2:4,0)*ddabl(zzcd,cd)
    end do
    eslsl(1,:)=eslsl(1,:)+osc(1,:)
    eslsl(2:4,:)=eslsl(2:4,:)+osc(2:4,:)*ddabl(zzab,ab)
end do
...
real(dp) :: fackl(0:1), facjl(0:1), facjk(0:1), vec(3)
...
integer(i4b) :: ab, cd, za, zb, zc, zd, zzab, zzcd, m, jj, k
