...
do zzab=1,nksqu(ab)
    ...
    ssp=sss*rbp
    s(2:4,1)=s(2:4,1)+sps
    s(1,2:4)=s(1,2:4)+ssp*ddabl(zzab,ab)
    fac1=hpm1(zzab,ab)*sss
    do ii=1,3
      do jj=1,3
        spp(ii,jj)=ssp(jj)*rap(ii)
    ...
    s(2:4,2:4)=s(2:4,2:4)+spp*ddabl(zzab,ab)
    ...
    t(2:4,1)=t(2:4,1)+tps
    tsp=rbp*tss+2._dp*qab(zzab,ab)*ssp
    t(1,2:4)=t(1,2:4)+tsp*ddabl(zzab,ab)
    fac3=fac2*fac1
    do ii=1,3
      do jj=1,3
    ...
    t(2:4,2:4)=t(2:4,2:4)+tpp*ddabl(zzab,ab)
    ...
    do nuc =1,natom
      ...
      vsp0=rbp*vssm(0)-rcp*vssm(1)
      vsp1=rbp*vssm(1)-rcp*vssm(2)
      vmat(1,2:4)=vmat(1,2:4)+vsp0*ddabl(zzab,ab)
      vmat(2:4,1)=vmat(2:4,1)+(rap*vssm(0)-rcp*vssm(1))
      do ii=1,3
        do jj=1,3
          vmat(1+ii,1+jj)=vmat(1+ii,1+jj)+(rap(ii)*vsp0(jj)-rcp(ii)*vsp1(jj))*ddabl(zzab,ab)
        end do
        vmat(1+ii,1+ii)=vmat(1+ii,1+ii)+hpm1(zzab,ab)*(vssm(0)-vssm(1))*ddabl(zzab,ab)
      end do
    end do
    fac3=-2._dp*PI*pm1(zzab,ab)*efac
    v=v+vmat*fac3
...
integer(i4b) :: nuc, ab, za, zb, zza, zzab, ii, jj, j, jm1, jp1, m, jrec
real(dp) :: fac1, fac2, fac3
...
integer(i4b) :: nuc, ab, zb, zzab, ii, jj, jp1, ip1,i,im1, m, jrec
...
do zzab=1,nksqu(ab)
    ...
    do jj=1,3
      smat(:,1+jj)=rbp(jj)*smat(:,1)*ddabl(zzab,ab)
      do i=5,10
        im1=minus1s(jj,i)
        if (im1.eq.0) cycle
        smat(i,1+jj)=smat(i,1+jj)+.5_dp*ddabl(zzab,ab)*smat(im1,1)*pm1(zzab,ab)*indexps(jj,i)
      end do
    end do
    csmat=csmat+smat(5:10,:)
    ...
    do i=5,10
      tmat(i,2:4)=rbp(:)*tmat(i,1)*ddabl(zzab,ab)+2._dp*q*smat(i,2:4)
    end do
    do i=5,10
      do jj=1,3
        jp1=jj+1
        im1=minus1s(jj,i)
        if (im1.eq.0) cycle
        tmat(i,jp1)=tmat(i,jp1)+hpm1(zzab,ab)*tmat(im1,1)*indexps(jj,i)*ddabl(zzab,ab)
      end do
    end do
    ctmat=ctmat+tmat(5:10,:)
    ! Finally use recursion to get p-d nuclear Coulomb integrals
    ...
    do nuc =1,natom
      ...
      do jj=1,3
        jp1=jj+1
        do i=5,10
          vmat(i,jp1,0)=(rbp(jj)*vmat(i,1,0)-rcp(jj)*vmat(i,1,1))*ddabl(zzab,ab)
          im1=minus1s(jj,i)
          if (im1.eq.0) cycle
          vmat(i,jp1,0)=vmat(i,jp1,0)+.5_dp*pm1(zzab,ab)*indexps(jj,i)*(vmat(im1,1,0)-vmat(im1,1,1))*ddabl(zzab,ab)
        end do
      end do
      cvmat=cvmat+vmat(5:10,:,0)
    end do
